...
@interface ISOTP_Receive_Valid : XCTestCase

@property(nonatomic,assign,readonly) Transceiver* isotp;

@end

@implementation ISOTP_Receive_Valid
...
-(void)testSingleShort {
    auto single = std::vector<uint8_t>{ 0x02, 0x3E, 0x00, padding, padding, padding, padding, padding };
...
-(void)testSingleMax {
    ...
    auto action = _isotp->didReceiveFrame(single);
    XCTAssertEqual(action.type, Transceiver::Action::Type::process);
    auto expected = std::vector<uint8_t> { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37 };
    XCTAssertEqual(action.data, expected);
}

-(void)testFirstConsecutive {
    auto first = std::vector<uint8_t> { 0x10, 0x08, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36 };
